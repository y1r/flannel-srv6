...
type SRv6Network struct {
	backend.SimpleNetwork
	BackendType         string
	routes              []netlink.Route
	neighs              []netlink.Neigh
	SM                  subnet.Manager
	GetRouteAndNeigh    func(lease *subnet.Lease) (*netlink.Route, *netlink.Neigh)
	Mtu                 int
	LinkIndex           int
}

func (n *SRv6Network) MTU() int {
	return n.Mtu
}

func (n *SRv6Network) Run(ctx context.Context) {
	wg := sync.WaitGroup{}
...
func (n *SRv6Network) handleSubnetEvents(batch []subnet.Event) {
	for _, evt := range batch {
		...
		case subnet.EventAdded:
			...
			route, neigh := n.GetRouteAndNeigh(&evt.Lease)
			...
			if err != nil {
				log.Warningf("Error adding neightbor %v to %v: %v", neigh, evt.Lease.Subnet, err)
			}
		...
		case subnet.EventRemoved:
			...
			if err := netlink.NeighDel(neigh); err != nil {
				log.Errorf("Error deleting neigh to %v: %v", evt.Lease.Subnet, err)
				continue
			}
...
func (n *SRv6Network) addToRouteList(route netlink.Route) {
...
func (n *SRv6Network) removeFromRouteList(route netlink.Route) {
...
func (n *SRv6Network) routeCheck(ctx context.Context) {
	for {
		select {
		case <-ctx.Done():
			return
...
func (n *SRv6Network) checkSubnetExistInRoutes() {
